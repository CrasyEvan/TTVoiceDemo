...
- (void)viewDidLoad {
    ...
    self.voiceView = [PQVoiceInputView pgq_reateVoiceViewWithRect:CGRectMake(100, 20, 100, 100)
                                                       voiceColor:nil
                                                      volumeColor:nil title:@"还没声音"
                                                         showType:0
    ...
    self.voiceView1 = [PQVoiceInputView pgq_reateVoiceViewWithRect:CGRectMake(100, 130, 50, 100)
                                                        voiceColor:[UIColor cyanColor]
                                                       volumeColor:[UIColor redColor]
                                                             title:@"按下说话"
                                                          showType:1 hidden:^(PQVoiceInputView * _Nullable view, NSString * _Nullable text, NSInteger type) {
        NSLog(@"上滑");
    }];
    [self.view addSubview:self.voiceView1];
    //
    self.voiceView2 = [PQVoiceInputView pgq_reateVoiceViewWithRect:CGRectMake(100, 250, 100, 100)
                                                        voiceColor:[UIColor cyanColor]
                                                       volumeColor:[UIColor greenColor] title:@"正在聆听..."
                                                          showType:2 hidden:^(PQVoiceInputView * _Nullable view, NSString * _Nullable text, NSInteger type) {
        NSLog(@"上滑");
    }];
